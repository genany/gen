...
import HjsonData, { hjsonToHjsonStr } from 'hjson2';
...
export default class ComponentTree extends React.Component {
  ...
  transTree(interInfo) {
    const hjson = new HjsonData().parse(interInfo.req);
    const root = hjson.getRootCommentJson();
    const tree = [];
    const rootNode = this.getTreeNode(root.ui[0].__componenntName);
  }
  getTreeNode = (title, key, value) => {
    return {
      title: '',
      key: '',
      value: ''
  ...
  render() {
    const { searchValue, expandedKeys, selectedKeys, autoExpandParent } = this.state;
