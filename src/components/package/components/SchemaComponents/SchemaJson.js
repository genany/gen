...
const mapping = (name, data, showEdit, showAdv, component) => {
  switch (data.type) {
    case 'array':
      return (
        <SchemaArray
...
class SchemaArray extends PureComponent {
  ...
  render() {
    const { data, prefix, showEdit, showAdv, component } = this.props;
      ...
      !_.isUndefined(data.items) && (
        <div className="array-type">
          ...
          <div className="option-formStyle">
            {mapping(prefixArray, items, showEdit, showAdv, componentList)}
          </div>
        </div>
      )
    );
...
class SchemaItem extends PureComponent {
  ...
  selectedComponent = (keys, component) => {
    let prefix = this.getPrefix();
    let key = [].concat(prefix).concat(keys);
    this.context.selectedComponent(key, component);
  };

  onChangeComponent = componentName => {
    let prefix = this.getPrefix();
    const key = [].concat(prefix).concat(['ui']);
    const value = { __componentName: componentName };
    this.Model.changeValueAction({ key, value });
  };

  // onChangeComponent = (keys, value) => {
  //   let prefix = this.getPrefix()
  //   let key = [].concat(prefix).concat(keys)
  //   // let value = e.target.value;
  //   this.Model.changeValueAction({ key, value })
  ...
  */
  ...
  render() {
    let { name, data, prefix, showEdit, showAdv, component } = this.props;
    ...
    return show ? (
      <div>
        <Row
          onClick={() => this.selectedComponent(['ui', '__componentName'], value.ui)}
          type="flex"
          justify="space-around"
          align="middle"
        >
          <Col
            span={this.context.isMock ? 8 : 10}
            className="col-item name-item col-item-name"
            style={this.__tagPaddingLeftStyle}
          >
            <Row type="flex" justify="space-around" align="middle">
          ...
          <Col span={3} className="col-item col-item-desc">
            <Input
              addonAfter={<Icon type="edit" onClick={() => this.handleShowEdit('label')} />}
              placeholder={LocaleProvider('label')}
              value={value.label}
          ...
          <Col span={2} className="col-item col-item-desc">
            <TreeSelect
              ...
              defaultValue={value.ui && value.ui.__componentName}
              value={value.ui && value.ui.__componentName}
          ...
          <Col span={3} className="col-item col-item-setting">
            ...
            <span className="delete-item" onClick={this.handleDeleteItem}>
              <Icon type="minus" className="close" />
            </span>
            {value.type === 'object' ? (
        ...
        <div className="option-formStyle">
          {mapping(prefixArray, value, showEdit, showAdv, componentList)}
        </div>
      </div>
    ) : null;
  }
...
const SchemaJson = props => {
  const item = mapping([], props.data, props.showEdit, props.showAdv, props.component);
